...
public class Utils {
    ...
    public static Map<String, Range> parseListToRangeMap(List<String> list) {
        ...
        list.stream().forEach(s -> {
            ...
            // min = 1.0, min2 = 1.00
            try {
                lowerBound = parts[1].isBlank() ? null : Double.parseDouble(min);
                upperBound = parts[2].isBlank() ? null : Double.parseDouble(max);
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException("The provided min or max value is not a number.");
            }

            if (lowerBound == null && upperBound == null) {
                throw new IllegalArgumentException("The provided lower bound and upper bound value are empty.");
            } else if (lowerBound == null) {
                map.put(configKey, Range.atMost(upperBound.doubleValue()));
            } else if (upperBound == null) {
                map.put(configKey, Range.atLeast(lowerBound.doubleValue()));
            } else {
                map.put(configKey, Range.closed(lowerBound.doubleValue(), upperBound.doubleValue()));
            }
        });
        return Map.copyOf(map);
    }
